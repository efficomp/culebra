...
class ACOFS2Tester(unittest.TestCase):
    """Test :py:class:`culebra.trainer.aco.ACOFS2`."""
    ...
    def test_init(self):
        ...
        # Try a custom value for the initial pheromone
        custom_initial_pheromone = 2
        trainer = ACOFS2(**params, initial_pheromone=custom_initial_pheromone)
        self.assertEqual(
            trainer.initial_pheromone[0], custom_initial_pheromone
        )
        ...
        for prob in invalid_probs:
            with self.assertRaises(TypeError):
                ACOFS2(**params, discard_prob=prob)
    ...
    def test_internals(self):
        ...
        # Check the choice info matrix
        self.assertFalse(trainer.pheromone is None)
        self.assertIsInstance(trainer.pheromone[0], np.ndarray)
        self.assertEqual(
            trainer.pheromone[0].shape,
            (
                trainer.species.num_feats,
                trainer.species.num_feats
            )
        ...
        self.assertTrue(
            np.all(trainer.initial_pheromone == trainer.pheromone[0])
        )
    ...
    def test_deposit_pheromone(self):
        ...
        for i in range(1000):
            ...
            for elite_ant in trainer._elite:
                org = elite_ant.path[-1]
                for dest in elite_ant.path:
                    self.assertGreater(trainer.pheromone[0][org][dest], 1)
                    self.assertGreater(trainer.pheromone[0][dest][org], 1)
                    org = dest
