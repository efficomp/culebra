...
* :class:`~culebra.fitness_func.abc.SingleObjectiveFitnessFunction`:
...
class SingleObjectiveFitnessFunction(FitnessFunction):
    ...
    def index(self) -> int:
        ...
        :param value: The new index. If set to :data:`None`,
            :attr:`~culebra.fitness_func.abc.SingleObjectiveFitnessFunction._default_index`
            is chosen
        :type value: int
        :raises TypeError: If *value* is not an integer number
        :raises ValueError: If *value* is a negative number
        """
        return self._default_index if self._index is None else self._index
