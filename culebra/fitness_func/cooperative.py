...
* :class:`~culebra.fitness_func.cooperative.FSSVCScorer`: Abstract base
...
from culebra.fitness_func.svc_optimization.abc import (
    SVCScorer,
    RBFSVCScorer
)
...
class FSSVCScorer(MultiObjectiveFitnessFunction):
    ...
    def __init__(
        ...
        *objectives: tuple[SingleObjectiveFitnessFunction, ...]
        ...
        :class:`~culebra.fitness_func.feature_selection.abc.FSClassificationScorer`
        instances using an
    ...
    def construct_solutions(
        ...
        sol: Solution,
        index: int,
        cooperators: Sequence[Solution | None]
    ) -> tuple[Solution, ...]:
        """Assemble the solution and cooperators.

           This fitness function assumes that:

             * *cooperators[0]*: Codes the SVC hyperparameters
               (C and gamma). Thus, it is an instance of
               :class:`culebra.solution.parameter_optimization.Solution`
             * *cooperators[1:]*: The remaining solutions code the
        ...
        :type cooperators: ~collections.abc.Sequence[~culebra.abc.Solution]
        ...
        sol_hyperparams = sol if index == 0 else cooperators[0]

        # Prototype solution for the final solution containing all the
        # features
        prototype_sol_features = sol if index == 1 else cooperators[1]
        ...
        for repr_index in range(1, num_cooperators):
            # Choose thge correct solution
            the_sol = (
                sol if repr_index == index else cooperators[repr_index]
            )
            # Get the features
