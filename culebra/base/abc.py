...
class Fitness(DeapFitness, Base):
    ...
    The concept of *similarity threshold* for each objective, introduced in
    [Gonzalez2021]_, considers that two solutions are similar (for a given
    objective) if their difference (for that objective) is below the similarity
    threshold (of that objective). If not defined, 0 will be used for each
    objective.
    """
...
class FitnessFunction(Base):
    ...
    class Fitness(Fitness):
        ...
        similarity :py:attr:`~base.Fitness.thresholds`) according to the
        fitness function.
        """
    ...
    def set_fitness_thresholds(
        ...
        Modifies the similarity :py:attr:`~Fitness.thresholds` of the
        :py:attr:`~FitnessFunction.Fitness` objects generated by this fitness
        function.

        :param thresholds: The new similarity thresholds. If only a single
            value is provided, the same threshold will be used for all the
